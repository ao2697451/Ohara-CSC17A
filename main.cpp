 ...
 *  This is Version 2; changes from Version 1 include working utility and railroad properties
 *  Concepts Added for Version 2: polymorphism/virtual member functions
 *  Line Count: 1017
 */
...
{
protected:
    int rent, price, owner, space, typeOwned;
    bool owned;
private:
...
Railroad::Railroad(string n, int s)
{
    setName(n);
    setSpace(s);
    setPrice(200);
...
Utility::Utility(string n, int s)
{
    setName(n);
    setSpace(s);
    setRent(50);
...
int getRent(Property *prop, int x){
    int rent;
    rent = prop->calcRent(x);
    return rent;
}
...
TurnInfo pTurn(int & drawn,PlayerInfo *players, int p, char board[], Property  properties[], Railroad railroads[],Utility utilities[],Chance *chanceDeck){
    ...
    cout<<"You have "<<player.numberOfUtilities<<" utilties\n";
    ...
    if(board[player.space]=='G')
        ...
        if(railroads[propNum].getOwned()==false){
            ...
            if(owner == player.name){
                ...
                rent = getRent(&railroads[propNum], players[ownerNum].numberOfRailroads);
        ...
        if(utilities[propNum].getOwned()==false){
            cout<<"This utility is unowned! Would you like to purchase it for $200?\n";
            cin>>yesNo;
            while(toupper(yesNo)!='Y'&&toupper(yesNo)!='N'){
                cout<<"Invalid entry, try again.\n";
                cin>>yesNo;
            ...
            if(owner == player.name){
                ...
                rent = getRent(&utilities[propNum], players[ownerNum].numberOfUtilities);
                cout<<"You need to pay "<<owner<<" $"<<rent<<endl;
                player.money-=rent;
                turn.money = rent;
                turn.player = ownerNum;
        ...
        if(properties[propNum].getOwner()==player.number){
            ...
            rent = getRent(&(properties[propNum]), 3);
...
TurnInfo aiTurn(int & drawn,PlayerInfo *players,int p, char board[], Property properties[], Railroad railroads[], Utility utilities[], Chance *chanceDeck){
    ...
    if(board[player.space]=='G')
        ...
        cout<<"You landed on "<<railroads[propNum].getName()<<endl;
        if(railroads[propNum].getOwned()==false){
            cout<<"This railroad is unowned! The AI purchases it for $200.\n";
        ...
        cout<<"You landed on "<<utilities[propNum].getName()<<endl;
        if(utilities[propNum].getOwned()==false){
            cout<<"This utility is unowned! The AI purchases it for $200.\n";
            cout<<"The AI is buying "<<utilities[propNum].getName()<<" for $200.\n";
            player.money-=utilities[propNum].getPrice();
            utilities[propNum].setOwned(true);
            utilities[propNum].setOwnersName(player.name);
            utilities[propNum].setOwner(player.number);
